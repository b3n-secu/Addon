...
class ModbusConfigGenerator:
    ...
    def generate_yaml(self, output_path=None):
        ...
        # Create YAML structure (list of devices for !include)
...
# ==========================================
#
# USAGE OPTION 1 (recommended - using !include):
#   Add this line to your configuration.yaml:
#   modbus: !include modbus_generated.yaml
#
# USAGE OPTION 2 (direct copy):
#   Add "modbus:" before the first device, then copy everything:
#   modbus:
#     - name: your_device
#       ...
#
        ...
        if output_path:
            try:
                ...
                if os.path.exists(output_path):
                    ...
                    logger.info(f"File verified: {output_path} ({file_size} bytes)")
                else:
                    logger.error(f"File not found after writing: {output_path}")

            except PermissionError as e:
                logger.error(f"Permission denied writing to {output_path}: {e}")
                raise
            except Exception as e:
                logger.error(f"Error writing configuration to {output_path}: {e}")
